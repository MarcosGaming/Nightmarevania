...
// ANIMATION
bool Animation::facingRight;

Animation::Animation() { facingRight = true; }
...
void Animation::setFacing(Entity* owner)
{
	...
	if (movement->getVelocity().x > 0.5f && !facingRight)
	{
		sprite->getSprite().scale(-1.0f, 1.0f);
		facingRight = true;
	}
	else if (movement->getVelocity().x < -0.5f && facingRight)
	{
		sprite->getSprite().scale(-1.0f, 1.0f);
		facingRight = false;
	}
